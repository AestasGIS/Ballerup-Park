...
elements_pol {
	element_id uuid pk def(uuid_generate_v4())
	base_type char def('F')
	element_group character varying
	element_type character varying
	label character varying
	geom bytea
	area float8 def(ST_Area(geom))
	perimeter float8 def(ST_Perimeter(geom))
	created_at timestamp with time zone def(now())
	created_by character varying def(CURRENT_USER)
	modified_at timestamp with time zone def(now())
	modified_by character varying def(CURRENT_USER)
}

elements_lin {
	element_id uuid pk def(uuid_generate_v4())
	base_type char def('L')
	element_group character varying
	element_type character varying
	label character varying
	geom bytea
	length float8 def(ST_Length(geom))
	created_at timestamp with time zone def(now())
	created_by character varying def(CURRENT_USER)
	modified_at timestamp with time zone def(now())
	modified_by character varying def(CURRENT_USER)
...
element_group_types {
	name character varying pk
}

element_types {
	name character varying pk
	group_name character varying > element_group_types.name
	base_type char
	label character varying
}
...
CREATE SCHEMA IF NOT EXISTS park_lookup;
CREATE SCHEMA IF NOT EXISTS park_elementer;
CREATE SCHEMA IF NOT EXISTS park_aktiviteter;
CREATE SCHEMA IF NOT EXISTS park_aktiviteter;

-- creation of parameters table 
SET search_path = park_lookup, public;

CREATE TABLE IF NOT EXISTS element_grupper (
    navn character varying PRIMARY KEY NOT NULL,
);
